...
$(function () {
                ...
                if (detalles.length > 0) {
                    if (response.total_pieza == 0) {
                        htmlPieza = `<div class="bg-white p-4 rounded-lg shadow-md w-full text-center">
                            <p class="text-gray-500">No hay piezas anexadas a esta venta.</p>
                        </div>`;
                    }

                    if (response.total_sala == 0) {
                        htmlSala = `<div class="bg-white p-4 rounded-lg shadow-md w-full text-center">
                            <p class="text-gray-500">No hay salas anexadas a esta venta.</p>
                        </div>`;
                    }
                        ...
                        if (detalle.fk_pieza == null) {
                            htmlSala += `<div class="bg-white p-4 rounded-lg shadow-md flex items-center justify-between w-full md:w-[48%] lg:w-[32%]">
                                    ...
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <input type="number" step="0.01" class="txtPrecio txtPrecio${detalle.id_detalle} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 text-center" value="${detalle.costo_unitario}" data-id="${detalle.id_detalle}">
                                    </div>
                                </div>`;
                        } else {
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    if (xhr.status === 422) {
                        ...
                        notyf.error("Ocurrió un error al crear la venta.");
                    }
                },
            });
        }
    ...
    $("#piezas").autocomplete({
        ...
        select: function (event, ui) {
            ...
            let precio = parseFloat(ui.item.precio_venta);
            if (isNaN(precio) || precio <= 0) {
                let costo =
                    parseFloat(ui.item.costo_cacastero) +
                    parseFloat(ui.item.costo_tapicero);
                precio = costo * 1.8;
            }
            $("#precio_pieza").val(precio.toFixed(2));
    ...
    $("#salas").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $("#precio_sala").val(precio.toFixed(2));
    ...
    $(document).on("click", ".btnQuitar", function (e) {
        ...
        if (txtCant <= 1) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let baseDeleteUrl = $('meta[name="delete_pieza"]').attr(
                        "content",
                    );
    ...
    let previousPrice = 0;
    $(document).on("focus", ".txtPrecio", function () {
    ...
    $(document).on("change", ".txtPrecio", function (e) {
        e.preventDefault();
        var id_detalle = $(this).attr("data-id");
        var precio = $(this).val();
        var txtCant = $(".txtCant" + id_detalle).val();
        ...
        Swal.fire({
            ...
            text: "El precio del detalle será actualizado.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            if (result.isConfirmed) {
                let baseUpdateUrl = $('meta[name="update_pieza"]').attr(
                    "content",
                );
                let finalUrl = baseUpdateUrl.replace("__ID__", id_detalle);
                finalUrl = finalUrl.replace("__CANT__", txtCant);
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            inputPrecio.val(previousPrice); // Revertir si hay error en respuesta
                            notyf.error(response.message);
                        }
                    },
                    error: function (xhr, status, error) {
                        inputPrecio.val(previousPrice); // Revertir si hay error AJAX
                        // Manejo de errores
                        var responseText = xhr.responseText;
                        if (xhr.status === 500) {
                            if (xhr.responseJSON.message) {
                ...
                inputPrecio.val(previousPrice); // Revertir si cancela
            }
        });
    });
    ...
    $(document).on("click", ".btn_imprimir", function (e) {
        ...
        var id = $(this).attr("data-id");
        Swal.fire({
            title: "¿Está seguro de imprimir esta venta?",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
    ...
    $(document).on("click", "#btnImprimir", function (e) {
        ...
        Swal.fire({
            title: "¿Está seguro de imprimir esta venta?",
            text: "Esta acción generará un documento PDF con los detalles de la venta que no se podrá revertir.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
    ...
    $(document).on("click", ".btnAnular", function (e) {
        ...
        Swal.fire({
            ...
            text: "Esta acción generará un documento PDF de anulación con los detalles de la venta que no se podrá revertir.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            if (result.isConfirmed) {
                var basePrintUrl = $('meta[name="print_anulada"]').attr(
                    "content",
                );
                let finalUrl = basePrintUrl.replace("__ID__", id);
                // Abrir como ventana emergente
