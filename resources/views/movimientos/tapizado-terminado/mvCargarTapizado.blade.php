...
@section('title', 'Creacion de Tapizado Terminado')

@section('content')

    <meta name="store" content="{{ route('tapizado-terminado.store') }}">
    <meta name="update" content="{{ route('tapizado-terminado.update') }}">
    <meta name="piezas" content="{{ route('piezas.getPiezasTrasladoDisponibles') }}">
    <meta name="salas" content="{{ route('salas.getSalasDisponiblesTraslado') }}">
    <meta name="store_pieza" content="{{ route('tapizado-terminado.saveDetalle') }}">
    <meta name="cargar_pieza" content="{{ route('tapizado-terminado.getDetalle', ['id' => '__ID__']) }}">
    <meta name="action" content="{{ $data['action'] }}">
    <meta name="delete_pieza" content="{{ route('tapizado-terminado.deleteDetalle', ['id' => '__ID__']) }}">
    <meta name="update_pieza"
        content="{{ route('tapizado-terminado.updateDetalle', ['id' => '__ID__', 'cant' => '__CANT__']) }}">
    <meta name="print" content="{{ route('tapizado-terminado.imprimirPreliminar', ['id' => '__ID__']) }}">
    <meta name="print_real" content="{{ route('tapizado-terminado.imprimirFinal', ['id' => '__ID__']) }}">
    <meta name="redirect" content="{{ route('tapizado-terminado.index') }}">
    <meta name="meses-url" content="{{ route('nota-pieza.meses') }}">

    <div class="container w-full p-4">
        <div class="w-full mb-4 flex justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Tapizado Terminado</h1>
                <p id="p-indicador" class="text-gray-600">
                    {{ $data['action'] == 'crear' ? 'Creación de documento' : 'Edición de documento' }} </p>
            </div>
            <div>
                <a id="btnNuevo" href="{{ route('tapizado-terminado.index') }}"
        ...
        <div id="default-tab-content">
            <div class="hidden p-4 bg-white rounded-lg shadow-md" id="profile" role="tabpanel"
                ...
                <form id="frmCrear"
                    data-id="{{ $data['action'] == 'crear' ? '' : $data['tapizadoTerminado']->id_movimiento }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" id="fecha" name="fecha"
                                ...
                                value="{{ $data['action'] == 'crear' ? date('Y-m-d') : date('Y-m-d', strtotime($data['tapizadoTerminado']->fecha_ingreso)) }}"
                                readonly>
                        </div>
                        <div>
                            ...
                            <input type="text" id="correlativo" name="correlativo"
                                class="bg-red-50 border border-red-300 text-red-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5"
                                value="{{ $data['action'] == 'crear' ? $data['correlativo'] : $data['tapizadoTerminado']->correlativo}}"
                                readonly>
                        </div>
                        <div class="col-span-1 sm:col-span-2  lg:col-span-2">
                            <label for="tapicero" class="block mb-2 text-sm font-medium text-gray-900">Tapicero <span
                                    class="text-red-500">(*)</span></label>
                            <select id="tapicero" name="tapicero"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                <option value="">Seleccione un tapicero</option>
                                @foreach ($data['tapicero'] as $tapicero)
                                    @if($data['action'] == 'editar' && $tapicero->id_trabajador == $data['tapizadoTerminado']->tapicero)
                                        <option value="{{ $tapicero->id_trabajador }}" selected>{{ $tapicero->nombre1 }}
                                            {{ $tapicero->nombre2 }} {{ $tapicero->apellido1 }} {{$tapicero->apellido2}}</option>
                                    @else
                                        <option value="{{ $tapicero->id_trabajador }}">{{ $tapicero->nombre1 }}
                                            {{ $tapicero->nombre2 }} {{ $tapicero->apellido1 }} {{$tapicero->apellido2}}</option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class=" col-span-1 sm:col-span-2  lg:col-span-4">
                            ...
                            <textarea type="text" id="comentario" name="comentario"
                                ...
                                placeholder="Ingrese el comentario">{{ $data['action'] == 'crear' ? '' : $data['tapizadoTerminado']->comentario }}</textarea>
                        </div>
                    </div>
...
@section('script')
    @vite(['resources/js/movimientos/mvTapizadoTerminado.js'])
@endsection