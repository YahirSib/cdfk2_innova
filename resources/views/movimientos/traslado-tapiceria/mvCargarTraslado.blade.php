...
@section('title', 'Creacion de Traslado a Tapiceria')
...
@section('content')
    ...
    <meta name="update_pieza"
        content="{{ route('traslado-tapiceria.updateDetalle', ['id' => '__ID__', 'cant' => '__CANT__']) }}">
    ...
    <div class="container w-full p-4">
        <div class="w-full mb-4 flex justify-between">
            ...
            <div>
                <a id="btnNuevo" href="{{ route('traslado-tapiceria.index') }}"
        ...
        <div id="default-tab-content">
            <div class="hidden p-4 bg-white rounded-lg shadow-md" id="profile" role="tabpanel"
                ...
                <form id="frmCrear"
                    data-id="{{ $data['action'] == 'crear' ? '' : $data['trasladoTapiceria']->id_movimiento }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" id="fecha" name="fecha"
                                ...
                                value="{{ $data['action'] == 'crear' ? date('Y-m-d') : date('Y-m-d', strtotime($data['trasladoTapiceria']->fecha_ingreso)) }}"
                                readonly>
                        </div>
                        <div>
                            ...
                            <input type="text" id="correlativo" name="correlativo"
                                class="bg-red-50 border border-red-300 text-red-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5"
                                value="{{ $data['action'] == 'crear' ? $data['correlativo'] : $data['trasladoTapiceria']->correlativo}}"
                                readonly>
                        </div>
                        <div class="col-span-1 sm:col-span-2  lg:col-span-2">
                            <label for="cacastero" class="block mb-2 text-sm font-medium text-gray-900">Cacastero <span
                                    class="text-red-500">(*)</span></label>
                            <select id="cacastero" name="cacastero"
                                ...
                                @foreach ($data['carpintero'] as $cacastero)
                                    @if($data['action'] == 'editar' && $cacastero->id_trabajador == $data['trasladoTapiceria']->cacastero)
                                        <option value="{{ $cacastero->id_trabajador }}" selected>{{ $cacastero->nombre1 }}
                                            {{ $cacastero->nombre2 }} {{ $cacastero->apellido1 }} {{$cacastero->apellido2}}</option>
                                    @else
                                        <option value="{{ $cacastero->id_trabajador }}">{{ $cacastero->nombre1 }}
                                            {{ $cacastero->nombre2 }} {{ $cacastero->apellido1 }} {{$cacastero->apellido2}}</option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class=" col-span-1 sm:col-span-2  lg:col-span-4">
                            ...
                            <textarea type="text" id="comentario" name="comentario"
                                ...
                                placeholder="Ingrese el comentario">{{ $data['action'] == 'crear' ? '' : $data['trasladoTapiceria']->comentario }}</textarea>
                        </div>
                    </div>
