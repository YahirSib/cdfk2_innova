...
<meta name="store" content="{{ route('traslado-tapiceria.store') }}">
<meta name="update" content="{{ route('traslado-tapiceria.update') }}">
<meta name="piezas" content="{{ route('piezas.getPiezasDisponibles') }}">
<meta name="salas" content="{{ route('salas.getSalasDisponibles') }}">
<meta name="store_pieza" content="{{ route('traslado-tapiceria.saveDetalle') }}">
<meta name="cargar_pieza" content="{{ route('traslado-tapiceria.getDetalle', ['id' => '__ID__']) }}">
<meta name="action" content="{{ $data['action'] }}">
<meta name="delete_pieza" content="{{ route('traslado-tapiceria.deleteDetalle', ['id' => '__ID__']) }}">
<meta name="update_pieza" content="{{ route('traslado-tapiceria.updateDetalle', ['id' => '__ID__', 'cant' => '__CANT__']) }}">
<meta name="print" content="{{ route('traslado-tapiceria.imprimirPreliminar', ['id' => '__ID__']) }}">
<meta name="print_real" content="{{ route('traslado-tapiceria.imprimirFinal', ['id' => '__ID__']) }}">
<meta name="redirect" content="{{ route('traslado-tapiceria.index') }}">
...
<div class="container w-full p-4">
    <div class="w-full mb-4 flex justify-between">
        ...
        <div>
            <a id="btnNuevo" href="{{ route('traslado-tapiceria.index') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center m-1">
                    Regresar
            </a>
        </div>
    </div>
    ...
    <div id="default-tab-content" >
        <div class="hidden p-4 bg-white rounded-lg shadow-md" id="profile" role="tabpanel" aria-labelledby="profile-tab">
            <form id="frmCrear" data-id="{{ $data['action'] == 'crear' ? '' : $data['trasladoTapiceria']->id_movimiento }}"  method="{{ $data['action'] == 'crear' ? 'POST' : 'PUT' }}">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <!-- Nombre del empleado-->
                    <div>
                        <label for="fecha" class="block mb-2 text-sm font-medium text-gray-900">Fecha Documento</label>
                        <input type="date" id="fecha" name="fecha" class="bg-red-50 border border-red-300 text-red-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5" value="{{ $data['action'] == 'crear' ? date('Y-m-d') : date('Y-m-d', strtotime($data['trasladoTapiceria']->fecha_ingreso)) }}" readonly>
                    </div>
                    <div>
                        <label for="correlativo" class="block mb-2 text-sm font-medium text-gray-900">N° Documento </label>
                        <input type="text" id="correlativo" name="correlativo" class="bg-red-50 border border-red-300 text-red-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5" value="{{ $data['action'] == 'crear' ? $data['correlativo'] : $data['trasladoTapiceria']->correlativo}}" readonly>
                    </div>
                    <div class="col-span-1 sm:col-span-2  lg:col-span-2">
                        ...
                        <select id="cacastero" name="cacastero" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            ...
                            @foreach ($data['carpintero'] as $cacastero)
                                @if($data['action'] == 'editar' && $cacastero->id_trabajador == $data['trasladoTapiceria']->cacastero)
                    ...
                    <div class=" col-span-1 sm:col-span-2  lg:col-span-4">
                        ...
                        <textarea type="text" id="comentario" name="comentario" class="bg-gray-50 h-auto border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Ingrese el comentario">{{ $data['action'] == 'crear' ? '' : $data['trasladoTapiceria']->comentario }}</textarea>
                    </div>
                </div>
        ...
        <div class="hidden " id="dashboard" role="tabpanel" aria-labelledby="dashboard-tab">
            ...
            <div class="flex gap-4 mb-4">
                <div class="cursor-pointer flex-1 p-4 rounded-2xl shadow hover:shadow-lg transition 
                            bg-white hover:bg-green-50 text-center card-detalle"
                    data-target="frmAnexarPieza">
                ...
                <div class="cursor-pointer flex-1 p-4 rounded-2xl shadow hover:shadow-lg transition 
                            bg-white hover:bg-blue-50 text-center card-detalle"
                    data-target="frmAnexarSala">
            ...
            <form class="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-10 gap-2 bg-white p-4 rounded-lg shadow-md mb-8" id="frmAnexarPieza" method="POST" id="detalle1" action="">
                <div class="md:col-span-3 lg:col-span-5 mb-2" >
                    <label for="piezas" class="block mb-2  md:text-left  text-sm font-medium text-gray-900">Pieza<span class="text-red-500">(*)</span> </label>
                    <input type="text" id="piezas" name="piezas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Buscar pieza">
                </div>
            ...
            <form class="hidden grid grid-cols-1 md:grid-cols-6 lg:grid-cols-10 gap-2 bg-white p-4 rounded-lg shadow-md mb-8" id="frmAnexarSala" method="POST" id="detalle2" action="">
                <div class="md:col-span-3 lg:col-span-5 mb-2" >
                    <label for="salas" class="block mb-2  md:text-left  text-sm font-medium text-gray-900">Salas<span class="text-red-500">(*)</span> </label>
                    <input type="text" id="salas" name="salas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Buscar sala">
                </div>
