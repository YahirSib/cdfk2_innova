...
class VentaSalaController extends Controller
{

    private $tipo_doc = 'VS'; // Agrupación de Salas
    private $tipo_mov = 'S'; // Tipo de movimiento para Nota de Pieza
    private $nombre_doc = 'Venta de Salas'; // Nombre del documento
    ...
    {
        return view('movimientos.venta-sala.mvVentaSala', ['menu' => $this->getMenu()]);
    }

    public function datatable(Request $request)
    {
        $ventaSala = Movimiento::query()
            ->select(
                'id_movimiento',
        ...
        if ($request->filled('mes')) {
            $ventaSala->whereMonth('fecha_ingreso', $request->mes);
        }

        return datatables()->of($ventaSala)
            ->filterColumn('cliente', function ($query, $keyword) {
                $sql = 'LOWER(cliente) LIKE ?';
            ...
            ->addColumn('acciones', function ($ventaSala) {

                $html = '<div class="flex justify-evenly items-center">';

                if ($ventaSala->estado == 'A') {
                    $html .= '<a data-id="' . $ventaSala->id_movimiento . '" href ="' . route('venta-sala.edit', ['id' => $ventaSala->id_movimiento]) . '" class="btn_editar btn btn-sm btn-primary cursor-pointer"><i class=" text-2xl text-yellow-600 hover:text-yellow-400 bx bxs-edit"></i></a>';
                    $html .= '<button data-id="' . $ventaSala->id_movimiento . '" class="btn_eliminar btn btn-sm btn-danger cursor-pointer"><i class=" text-2xl text-red-600 hover:text-red-400 bx bxs-trash"></i></button>';
                } else if ($ventaSala->estado == "I") {
                    $html .= '<a data-id="' . $ventaSala->id_movimiento . '" data-estado="' . $ventaSala->estado . '" class="btnReporte btn btn-sm btn-primary cursor-pointer"><i class="bx bxs-file-pdf text-2xl text-blue-600 hover:text-blue-400"></i></a>';
                    $html .= '<button data-id="' . $ventaSala->id_movimiento . '" class="btnAnular btn btn-sm btn-danger cursor-pointer"> <i class="bx bx-revision text-2xl text-red-600 hover:text-red-400"></i> </button>';
                } else {
                    $html .= '<a data-id="' . $ventaSala->id_movimiento . '" data-estado="' . $ventaSala->estado . '" class="btnReporte btn btn-sm btn-primary cursor-pointer"><i class="bx bxs-file-pdf text-2xl text-blue-600 hover:text-blue-400"></i></a>';
                }
    ...
    {
        $ventaSala = Movimiento::findOrFail($id);
        $data['action'] = 'editar';
        $ventaSala->correlativo = $this->getTipoDoc() . '-' . str_pad($ventaSala->correlativo, 5, '0', STR_PAD_LEFT);
        $data['ventaSala'] = $ventaSala;

        if ($ventaSala->estado != 'A') {
            return redirect()->route('venta-sala.index')->with('error', 'No se puede editar una ' . $this->getNombreDoc() . ' que no está activa.');
        }

        return view('movimientos.venta-sala.mvCargarVenta', ['data' => $data, 'menu' => $this->getMenu()]);
    }

    public function destroy($id)
    {
        $ventaSala = Movimiento::find($id);
        if (!$ventaSala) {
        ...
        $detalles = Detalle::where('fk_movimiento', $ventaSala->id_movimiento)->get();
    ...
    public function imprimirHistorico($id)
    {
        ...
        $data['ventaSala'] = $ventaSala;
        $data['correlativo'] = $ventaSala->correlativo_formateado;
        $data['fecha_ingreso'] = $ventaSala->fecha_ingreso_formateada;
        $data['cliente'] = $ventaSala->cliente;
        $detalles_pieza = $ventaSala->detalles()->where('fk_pieza', '!=', null)->with('pieza')->get();
        $detalles_sala = $ventaSala->detalles()->where('fk_sala', '!=', null)->with('sala')->get();
        $total = $ventaSala->totalizar();
        $totalUnidades = $ventaSala->totalizarUnidades();

        return $this->renderPDF($ventaSala, $detalles_pieza, $detalles_sala, $total, $totalUnidades, $data);
    }
    ...
    public function imprimirAnular($id)
    {
        try {
            ...
            $data['fecha_ingreso'] = $ventaSala->fecha_ingreso_formateada;
            $data['cliente'] = $ventaSala->cliente;
            $detalles_piezas = $ventaSala->detalles()->where('fk_pieza', '!=', null)->with('pieza')->get();
            $detalles_sala = $ventaSala->detalles()->where('fk_sala', '!=', null)->with('sala')->get();
            $total = $ventaSala->totalizar();
            $totalUnidades = $ventaSala->totalizarUnidades();

            $ventaSala->estado = 'Z';
            $ventaSala->fecha_anulacion = now();
            $ventaSala->save();
            ...
            DB::commit();
            return $this->renderPDF($ventaSala, $detalles_piezas, $detalles_sala, $total, $totalUnidades, $data);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function renderPDF($entrada, $detalles_pieza, $detalles_sala, $total, $totalUnidades, $data)
    {
        ...
        $pdf = Pdf::loadView('movimientos.venta-sala.pdfVS', compact('entrada', 'detalles_pieza', 'detalles_sala', 'total', 'totalUnidades', 'data'));
        $pdf->setPaper('A4', 'portrait');

        return $pdf->stream('venta-sala' . $entrada->correlativo . '.pdf');
    }
    ...
    public function store(Request $request)
    {
        ...
        $ventaSala = new Movimiento();

        $ventaSala->tipo_mov = $this->getTipoMov(); // Asignar tipo de movimiento
        $ventaSala->tipo_doc = $this->getTipoDoc(); // Asignar tipo de documento
        $ventaSala->fecha_ingreso = now(); // Fecha actual
        $ventaSala->total = 0; // Inicializar total en 0
        $ventaSala->comentario = $request->comentario; // Asignar comentario
        $ventaSala->estado = 'A'; // Asignar estado activo
        $numero_cor = (int) preg_replace('/[^0-9]/', '', $request->correlativo);
        $ventaSala->correlativo = $numero_cor; // Asignar correlativo
        $ventaSala->cliente = $request->cliente; // Asignar el ID del carpintero

        try {
            DB::beginTransaction();
            $ventaSala->save();
            DB::commit();
            return response()->json(['success' => true, 'message' => $this->getNombreDoc() . ' registrada con exito.', 'redirect' => route('venta-sala.edit', ['id' => $ventaSala->id_movimiento])]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request)
    {
        ...
        $ventaSala = Movimiento::findOrFail($request->id_movimiento);
        $ventaSala->cliente = $request->cliente; // Asignar el ID del carpintero
        $ventaSala->comentario = $request->comentario; // Asignar comentario
